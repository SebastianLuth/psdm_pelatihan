...
import {
  deleteWasiatKarpelById,
  getAllDataWasiatKarpelAdmin,
  getAllDataWasiatKarpelSuperAdmin,
} from "@/service/wasiat";
...
export const DataWasiatKarpelComponent = () => {
  ...
  const handleDeleteSkmbt = async (skmbtId: number) => {
    try {
      await deleteWasiatKarpelById(skmbtId);
    } catch (error) {
      console.error(error);
      Swal.fire(
        "Gagal!",
  ...
  const handleEditSkmbt = (id: number) => {
    router.push(`/skmbt/karpel/edit/${id}`);
  };

  const fetchDataSkmbtSuperAdmin = async () => {
    try {
      const result = await getAllDataWasiatKarpelSuperAdmin();
      setSkmbtData(
  ...
  const fetchDataSkmbtAdmin = async () => {
    try {
      const result = await getAllDataWasiatKarpelAdmin();
        ...
        <>
          <div className="flex justify-between">
            <h4 className="p-4 text-2xl font-semibold text-gray-800 dark:text-gray-100">
              Data Semua Wasiat Karpel Pra Pensiun
            </h4>
            <Breadcrumb />
          </div>
          ...
          <div className="relative overflow-hidden rounded-xl border border-gray-300 bg-white shadow-xl backdrop-blur-lg dark:border-gray-700 dark:bg-gray-900/70">
            <div className="px-6">
              <div className="flex items-center justify-between space-x-4 py-4">
                ...
                <Link
                  href="/skmbt/karpel/create-skmbt"
                ...
                >
                  Tambah Wasiat Pra Pensiun Karpel
                </Link>
              </div>

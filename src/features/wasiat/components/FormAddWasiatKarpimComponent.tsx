...
import { addWasiatKarpim } from "@/service/wasiat";
...
export const FormAddWasiatKarpimComponent = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    try {
      await addWasiatKarpim(formData, {
        pdf_skmbt: getFile(SkmbtRef),
        pdf_skmbt2: getFile(SkmbtRef2),
      });
      setSuccess(true);
      setFormData({
        nama: "",
      ...
      <div className="mb-8 flex justify-between border-b border-gray-200 pb-4 dark:border-gray-700">
        <div>
          <h2 className="text-3xl font-bold text-gray-800 dark:text-white">
            Tambah Wasiat Pra Pensiun dari Karpim
          </h2>
          <p className="mt-1 text-gray-500 dark:text-gray-400">
            Lengkapi form berikut untuk menambahkan Wasiat Pra Pensiun dari
            Karpel baru
          </p>
        </div>
        <Breadcrumb />

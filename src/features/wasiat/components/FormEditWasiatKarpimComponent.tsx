...
export const FormEditWasiatKarpimComponent = () => {
  ...
  const fetchDetailDataSKMBT = useCallback(async () => {
    try {
      const response = await getDetailDataWasiatKarpimById(Number(skmbtId));
      setSkmbtData(response);

      // Set juga ke formData
      setFormData({
        nama: response.nama || "",
        niksap: response.niksap?.toString() || "",

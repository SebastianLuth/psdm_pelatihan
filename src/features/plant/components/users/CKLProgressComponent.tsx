...
import { getDataCKLUserByNIKSAP } from "@/service/plant";
import {
  BookmarkCheck,
  Building,
  Dumbbell,
...
export const CKLProgressComponent = () => {
  ...
  const fetchDataCKL = useCallback(async () => {
    try {
      const niksap = userData?.username || "";
      const result = await getDataCKLUserByNIKSAP(niksap);
      setCklData(result);
    } catch (error) {
          ...
          <>
            <div className="flex flex-col gap-2">
              ...
              <p className="p-4 text-sm text-gray-600 dark:text-gray-400">
                Ini Halaman untuk menilai kemampuan karyawan pimpinan, Segera
                isi semua nya
              </p>
            </div>

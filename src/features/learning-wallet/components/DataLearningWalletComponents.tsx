...
import {
  deleteRealisasiLWUserByLWId,
  getAllRealisasiLWAdmin,
  getAllRealisasiLWUser,
  updateStatusRealisasiLWUserByLWId,
...
export const RealisasiDataLearningWalletComponent = () => {
  ...
  const [dataDetailRKAPLWByUser, setDataDetailRKAPLWByUser] = useState<
    dataRKAPLWUser[]
  >([]);
  ...
  const fetchAllLearningWalletAdmin = async () => {
    try {
      const result = await getAllRealisasiLWAdmin();
  ...
  const fetchAllLearningWalletUser = useCallback(async () => {
    try {
      ...
      const result = await getAllRealisasiLWUser(username);
      setDataDetailRKAPLWByUser(result.dataRKAPLWUser);
      setLearningWalletUser(result.data);
    } catch (error) {
      setError(true);
    } finally {
      setLoading(false);
  ...
  const handleDeleteRealisasiLWUser = async (id: number) => {
    try {
      const result = await deleteRealisasiLWUserByLWId(id);
      Swal.fire({
        icon: "success",
        title: "Berhasil",
        text: result,
  ...
  const handleUpdateStatusLW = async (
    status: string,
    niksap: string | number,
    lwId: number,
  ) => {
    try {
      await updateStatusRealisasiLWUserByLWId(status, niksap, lwId);
          ...
          <div className="relative overflow-hidden rounded-xl border border-gray-300 bg-white shadow-xl backdrop-blur-lg dark:border-gray-700 dark:bg-gray-900/70">
                        ...
                        >
                          ...
                          <td className="whitespace-nowrap px-6 py-4">
                            <div className="flex items-center">
                              ...
                              <p className="ml-2 text-sm text-gray-900">
                                Rp.{" "}
                                {learningWallet.biaya_pelatihan.toLocaleString()}
                              </p>
                            </div>
                          </td>
                          <td className="whitespace-nowrap px-6 py-4">
                          ...
                          <td className="whitespace-nowrap px-6 py-4">
                            <span
                              ...
                              `}
                            >
                              {learningWallet.status.charAt(0).toUpperCase() +
                                learningWallet.status.slice(1)}
                            </span>
                          </td>
            ...
            <div className="flex items-center justify-between p-8 text-sm text-gray-500">
              <span>
                ...
                {Math.min(endIndexAdmin, totalEntriesAdmin)} of{" "}
                {totalEntriesAdmin} entries
              </span>
            ...
            <div className="bg-white px-4 py-4 md:px-8 md:py-7 xl:px-10">
              <div className="rounded-lg bg-white p-6 shadow-sm">
                <div className="flex flex-col gap-6">
                  ...
                  <div className="w-full rounded-lg border border-gray-200 bg-gray-50 p-5 sm:w-auto">
                      ...
                      <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
                        <div className="space-y-2">
                          <p className="text-sm text-gray-600">
                            RKAP Learning Wallet
                          </p>
                          <p className="text-lg font-medium text-indigo-700">
                            Rp
                            {dataDetailRKAPLWByUser[0].biaya_rkap_lw.toLocaleString()}
                          </p>
                          ...
                          <p className="text-lg font-medium text-indigo-700">
                            {dataDetailRKAPLWByUser[0].rkap_jpl.toLocaleString()}{" "}
                            JPL
                          </p>
                        </div>
                        ...
                        <div className="space-y-2">
                          <p className="text-sm text-gray-600">
                            Sisa Learning Wallet
                          </p>
                          <p className="text-lg font-medium text-indigo-700">
                            Rp
                            {dataDetailRKAPLWByUser[0].sisa_biaya_lw.toLocaleString()}
                          </p>
                          ...
                          <p className="text-lg font-medium text-indigo-700">
                            {dataDetailRKAPLWByUser[0].sisa_jpl.toLocaleString()}{" "}
                            JPL
                          </p>
                        </div>
                      </div>
                  ...
                  <div className="flex justify-between gap-4 sm:flex-col">
                    ...
                    <div className="flex rounded-full bg-gray-100 p-1">
                      ...
                      <a
                        ...
                        href="javascript:void(0)"
                      >
                        <div className="rounded-full px-6 py-2 text-gray-600 transition-colors duration-200 hover:bg-white">
                    ...
                    {/* Add Button */}
                    <Link
                      href="/learning-wallet/create"
                      className="w-full sm:w-auto"
                    >
                      <button className="flex w-full items-center justify-center rounded-md bg-indigo-600 px-6 py-3 text-sm font-medium text-white shadow-sm transition-colors duration-200 hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2">
              ...
              <div className="mt-7 overflow-x-auto">
                <table className="w-full whitespace-nowrap">
                  <tbody>
                    {Array.isArray(currentDataUser) &&
                    currentDataUser.length > 0 ? (
                      ...
                      <tr className="h-16 rounded border border-gray-100 focus:outline-none">
                ...
                <div className="flex items-center justify-between p-8 text-sm text-gray-500">
                  <span>
                    {" "}
                    Showing {startIndexUser + 1} to{" "}
                    {Math.min(endIndexUser, totalEntriesUser)} of{" "}
                    {totalEntriesUser} entries
                  </span>
                  <div className="space-x-2">
                    <button
                      className="rounded-lg bg-gray-200 px-3 py-1 transition hover:bg-gray-300"

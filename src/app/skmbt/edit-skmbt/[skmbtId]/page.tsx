...
const FormEditSKMBTPage = () => {
  ...
  const { skmbtId } = useParams();

  const fetchDetailDataSKMBT = useCallback (async () => {
    try {
      const response = await axios.get(`http://localhost:5000/api/skmbt/admin/${skmbtId}`, {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await axios.put(`http://localhost:5000/api/skmbt/admin/${skmbtId}`, data,{
        headers: {
...
export default FormEditSKMBTPage;
